    ...
    public partial class Form7 : Form
    {
        ...
        private void UpdataDateGridViewEmployee()
        {
            ...
            for (int i = 0; i < dtgoods.Rows.Count; i++)
            {
                ...
                dataGridView1[2, i].Value = dtgoods.Rows[i]["员工编号"];
                dataGridView1[3, i].Value = dtgoods.Rows[i]["品牌"];
                dataGridView1[4, i].Value = dtgoods.Rows[i]["商品名称"];
                dataGridView1[5, i].Value = dtgoods.Rows[i]["价格"];
                dataGridView1[6, i].Value = dtgoods.Rows[i]["库存"];
            }
        }
